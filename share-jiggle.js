...
function startJiggle() {
  ...
  /* After 3 s, stop animation and show answer ─────────────────────── */
  setTimeout(() => {
    window.crca.stop();
    flash.remove();

    /*  ↓↓↓  NEW: replicate normal-version inline styles  ↓↓↓  */
    Object.assign(resultDiv.style, {
      ...
      transform: 'translate(-50%, -50%)',
      zIndex: 30,
      fontSize: (isMobile() || isTablet()) ? '3em' : '15em'
    });
    /*  ↑↑↑  END NEW STYLES  ↑↑↑  */
...
function handleShake(e) {
  ...
  if (Date.now() - lastShake < 1000) return;
